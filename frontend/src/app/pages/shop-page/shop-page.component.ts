...
export class ShopPageComponent {
  ...
  unitBarRecord(item: ShopUnitItem): UnitRecord {
    return {
      ...
      total_attack: item.total_attack ?? 0,
      total_defense: item.total_defense ?? 0,
      total_precision: item.total_precision ?? 5,
      total_resolve: item.total_resolve ?? 5,
      current_hp: item.max_hp ?? 1,
      max_hp: item.max_hp ?? 1,
      xp: 0,
