...
describe('Camp run lifecycle', () => {
  ...
  function bootstrap(active = false): GameBootstrapData { return {
    ...
    progression: { unlock_ids: [] }, active_squad: { id: '31', name: 'Raiders', is_active: true,
    ...
    active_run: active ? { id: '41', region_id: 'region.the_farm', squad_id: '31', status: 'active' } : null,
  }; }
  ...
  it('prevents duplicate submission while a start is in flight', async () => {
    ...
    const first = screen.startFarm(); const second = screen.startFarm();
  ...
  it('resumes an active run without issuing another start request', () => {
    const { api, enterRun, screen } = harness(true); screen.resumeFarm();
