...
final class GameplayUnitDetailsEndpointTest extends BattleFlowIntegrationCase
{
  ...
  public function testSellDiceAllowsUnassignedDiceButBlocksAbilitySlotAssignments(): void
  {
    ...
    $unassignedDiceId = $this->insertDiceInstance($userId, $this->pickAnyDiceDefinitionId());
    ...
    $unassignedSell = $this->invoke(fn() => $controller->sellDice((string)$unassignedDiceId));
    $this->assertSame(200, $unassignedSell['status'], json_encode($unassignedSell['body']));
    $this->assertSame(
      '0',
      (string)$this->scalar('SELECT COUNT(*) FROM `dice_instances` WHERE `id` = ?', [$unassignedDiceId])
    );
